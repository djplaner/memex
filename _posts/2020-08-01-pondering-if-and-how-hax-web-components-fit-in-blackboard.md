...
</style> So look what I've done inside my 

[Blackboard Learn][1] sandpit site. ![Initial example of magic script web components in Blackboard][2] It probably doesn't look that exciting. A bit of HTML and a couple of animated GIFS. Anyone could do that, right? Not quite. Following explains why this is a bit of game changer and explores how it might be leveraged in work with Blackboard. 
## It's not the what, it's the how ...And what the how makes possible The core of this image is enabled by four different 

[web components][3] 
1.  [grid-place][4] - provides an easy way to display the three SpongeBob images in a grid
2.  [meme-maker][5] - overlays the meme-like words (i.e. ARE, YOU, READY?) onto the SpongeBob images (no image manipulation required)
3.  [hax-logo][6]
4.  [type-writer][7] - provides the "type-writer" animation of "Any tag you type here that is listed..." A web component is a nicely encapsulated bit of code. Code that provides some specific functionality. For example, the ability to make a meme. i.e. take any image on the web and overlay words of my choosing on that image. (e.g. the three times SpongeBob's happy dance gif was used in the image above. No online meme maker was used in the construction of the above Blackboard-based content. Instead - like all web components - I used a specially created HTML tag. Just like any other HTML tag, but provide this unique meme making functionality. What's more I could use this functionality as many times as I want. For example, I could add a meme with happy dance SpongeBob saying 

["hello world"][8] To do this I would (and did) add the following HTML to my Blackboard page. `HTML
...
toptext="happy dance GIF by SpongeBob SquarePants"></p>` Which produces the following (all on the same Blackboard page). ![Hello World Happy Dance][9] **Note:** The *meme-maker* tag wouldn't work without the *p* tag around it. Perhaps a Blackboard thing, or perhaps an artefact of the kludge I've used to get it to work in Blackboard. The **meme-maker** web component includes code that knows how to take the values I've placed in the **top-text** and **bottom-text** attributes and overlay them onto the image I've specified in **image-url**. Change those attributes and I can create a new "meme". For example, something a little more HAX. ![Joker Hello World][10] 
...
[this JSON file][11]. A list that includes: various types of charts; more layout components like the grid; players for various types of multimedia; a discussion form; rich text editor; and, much, much more. Thanks to [the magic script][12] I just include the right HTML tags and it all happens as if by magic. ![It is Magic - Magic | Make a Meme][13] **TODO** I do need to find out if and where the docs are for the various components. The NPM pages and git repo aren't doing it for a lowly end user. 
...
3.  Add a [Javascript function][18] to decode the string into the item.
...
[The Content Interface][25] is the attempted soluton to the content authoring problem as part of [another attempt to share design knowledge for digital learning][26]. With the Content Interface authors use Microsoft Word (a tool many are comfortable with and which provides various authoring functionality) to create course materials. They use Word styles to semantically mark up the objects in their materials. The Word document is converted to HTML and pasted into Blackboard. A Javascript then transforms the semantically marked up HTML in various ways. 
#### The simple approach - Implemented One of the styles supported by the Content Interface is the 

*embed* style. It's used to include HTML (e.g. the embed code for a YouTube video) in the Word document which is then turned into HTML in Blackboard that is displayed (e.g. the YouTube video). If the magic script Javascript is added to the Content Interface javascript then it should be possible to embed web component HTML in the Word document and have it displayed via the Content Interface. ![Magic script, web components and the Content Interface working together][27] 
...
![Web components in the Card Interface][30] This is actually an unexpected outcome. A bit of tidying up would enable this to work somewhat. But of questionable value. 
...
*   Ponder the implications of web components in terms of the NGDLE/VLE and local strategic directions.
 ...
 [2]: haxImages/happyDance.gif
 [3]: https://developer.mozilla.org/en-US/docs/Web/Web_Components
 [4]: https://www.npmjs.com/package/@lrnwebcomponents/grid-plate
 [5]: https://www.npmjs.com/package/@lrnwebcomponents/meme-maker
 [6]: https://www.npmjs.com/package/@lrnwebcomponents/hax-logo
 [7]: https://www.npmjs.com/package/@lrnwebcomponents/type-writer
 [8]: https://en.wikipedia.org/wiki/%22Hello,_World!%22_program
 [9]: haxImages/HelloWorldHappyDance.gif
 [10]: haxImages/JokerHelloWorld.png
 [11]: https://cdn.webcomponents.psu.edu/cdn/wc-registry.json
 [12]: https://dev.to/btopro/uwc-part-3-the-magic-script-122a
 [13]: https://media.makeameme.org/created/its-magic-5c9ab8.jpg
 [14]: https://djon.es/hax.html
 [15]: https://codepen.io/btopro/pen/yLNmVbw
 [16]: https://en.wikipedia.org/wiki/Kludge
 [17]: https://www.w3schools.com/html/html_entities.asp
 [18]: https://stackoverflow.com/questions/7394748/whats-the-right-way-to-decode-a-string-that-has-special-html-entities-in-it/7394787#7394787
 [19]: https://haxtheweb.org
 [20]: https://www.youtube.com/watch?v=9Mq6J6U9K9Q&list=PLJQupiji7J5eTqv8JFiW8SZpSeKouZACH&index=11&t=0s
 [21]: haxImages/haxEditingBlackboardPage.png
 [22]: https://youtu.be/9Mq6J6U9K9Q?t=429
 [23]: https://plugins.trac.wordpress.org/browser/haxtheweb/tags/3.9.4/js/hax-the-press.js
 [24]: https://github.com/AllTheDucks/jshack-v1/wiki
 [25]: https://djon.es/blog/2019/08/08/exploring-knowledge-reuse-in-design-for-digital-learning-tweaks-h5p-constructive-templates-and-casa/#contentInterface
 [26]: https://djon.es/blog/2019/11/28/how-to-share-design-knowledge-in-design-for-digital-learning/
 [27]: haxImages/WebComponentsInContentInterface.gif
 [28]: https://mytasker.com/blog/parallax-scrolling-website
 [29]: https://djon.es/blog/2019/08/08/exploring-knowledge-reuse-in-design-for-digital-learning-tweaks-h5p-constructive-templates-and-casa/#cardInterface
 [30]: haxImages/WebComponentsInCardInterface.gif