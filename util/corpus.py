...
#        print(f"Link {link} -> {absPath}")
    ...
    def generateRelativePath( fromPath, toPath):
        """
        Given two full paths, return a string that shows the relative from the fromPath to the toPath
        ...
        path_from = pathlib.Path(fromPath)
        path_to = pathlib.Path(toPath)

        return path_from.relative_to( path_to, walk_up=True)
    ...
    def saveBubble(bubble:dict):
        ...
        with open(bubble['filePath'], 'w', encoding="utf-8-sig") as f:
            ...
            ## TODO: need to add the linkdefs
            f.write("[//begin]: # \"Autogenerated link references for markdown compatibility\"\n")
            for linkDefPath in bubble['linkDefs'].keys():
                linkDef = bubble['linkDefs'][linkDefPath]
                relativePath = generateRelativePath( bubble['filePath'], linkDef)
            
                f.write(f"[{linkDef['text']}]: {relativePath} \"{linkDef['description']}\"\n")

            f.write("[//end]: # \"Autogenerated link references\"\n")
