"""
FILE:   category.py
PURPOSE: Use mkdocs-gen-file to generate blog category page, showing all posts belonging to a category 

Relies on categories.json (or some such) file that lists all of the categories

- Will read that file and get all categories
- For each category, create a markdown file in blog/category/
- Each page lists the posts that belong in that category
- Maybe need a template for this type of page

- Can be something similar used for archives and tags


- Will need to identify which posts belong to a category from frontmatter
