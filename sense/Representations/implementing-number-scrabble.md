...
As a web developer with a strong focus on accessibility and visual design write a 2-player version of Herb Simon's number scrabble game using HTML, Javascript and CSS. Provide the HTML, Javascript, and CSS to implement the game. Include the CSS and Javascript

Each player takes turns choosing a card from a series of cards numbered 1 to 9. The first player to have 3 cards that add up to 15 wins. Otherwise the game is a tie. 

The game you write should take place on a single web page with three main sections: 1) available cards, 2) player 1's selected cards and score, and 3) player 2's selected cards and score.

Visually the cards should resemble playing cards of the diamond suit. The ace card represents the number 1, the 2 card the number to, and so on. Overall the visual design of the game should be heavily inspired by skeuomorphic design.

When an available card (not already selected) is selected by a player your game should:
...
2. In the available cards section, visually modify the card to show the back of the card with the number of player 1 or player 2 (depending on who selected the card) on the back of the card.
3. Add the card to the selected cards section for the player who selected the card.
4. Update the score for the player who selected the card.
...
Ensure that the game meets to the highest levels WCAG accessibility standards.
